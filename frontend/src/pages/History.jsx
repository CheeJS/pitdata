...
export default function History() {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(`${API_BASE}/api/races?year=${selectedYear}`)
            .then(res => {
                setRaces(res.data || []);
                setLoading(false);
            })
    ...
    useEffect(() => {
        ...
        setLoading(true);
        axios.get(`${API_BASE}/api/results/${selectedRaceId}`)
            .then(res => {
                            ...
                            <div className="flex-1 overflow-y-auto px-4 pb-20">
                                {loading ? (
                                    ...
                                    <div className="relative pt-4">
                                        {/* Timeline Line */}
                                        <div className="absolute left-[15px] top-0 bottom-0 w-[2px] bg-gradient-to-b from-f1-red via-gray-300 to-gray-300" />

                                        {Object.entries(racesByMonth).map(([month, monthRaces]) => (
                                            <div key={month} className="mb-6">
                                                {/* Month Header */}
                                                <div className="flex items-center gap-3 mb-3 sticky top-0 bg-f1-light/95 backdrop-blur-sm py-2 z-10">
                                                    <div className="w-8 h-8 rounded-none bg-white border-2 border-black flex items-center justify-center relative z-10">
                                                        <Calendar size={12} className="text-gray-500" />
                                                    </div>
                                                    <span className="text-xs font-heading text-black uppercase tracking-wider">{month}</span>
                                                </div>
                        ...
                        <div className="hidden md:flex flex-col flex-1 p-6 overflow-hidden">
                            ...
                            <div className="flex-1 overflow-y-auto">
                                {loading && !races.length ? (
                                    ...
                                    <div className="relative">
                                        <div className="absolute left-[19px] top-0 bottom-0 w-[2px] bg-gradient-to-b from-f1-red via-gray-300 to-gray-300" />

                                        {Object.entries(racesByMonth).map(([month, monthRaces], monthIdx) => (
                                            <div key={month} className="mb-8">
                                                <div className="flex items-center gap-4 mb-4 sticky top-0 bg-f1-light/95 backdrop-blur-sm py-2 z-10">
                                                    ...
                                                    <span className="text-sm font-heading text-black uppercase tracking-wider">{month}</span>
                                                </div>
            ...
            <div className="bg-white border-2 border-black p-6 flex justify-between items-end">
                ...
                {activeTab === 'R' && data.winner && (
                    <div className="text-right">
                        ...
                        <div className="text-2xl font-bold text-black">{data.winner}</div>

    ...
    if (!isNicknameSet) {
        return (
            <div className="flex h-[60vh] items-center justify-center">
                <div className="bg-[#1A1A20] p-8 rounded-2xl border border-[#2A2A30] w-full max-w-md text-center">
                    <div className="w-16 h-16 bg-f1-red/10 rounded-full flex items-center justify-center text-f1-red mx-auto mb-4">
                        <Users size={32} />
                    </div>
                    ...
                    <input
                        className="w-full bg-[#0B0B0F] border border-[#2A2A30] rounded-lg p-3 text-white mb-4"
                    ...
                    <button
                        ...
                        className="w-full bg-f1-red py-3 rounded-lg font-bold text-white disabled:opacity-50"
                    >
                        Join Session
                    </button>
                </div>
        ...
        <div className="max-w-4xl mx-auto h-[calc(100vh-6rem)] flex flex-col">
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-3">
                    {view === 'thread' && (
                        <button onClick={() => setView('feed')} className="p-2 hover:bg-white/10 rounded-full">
                            <ChevronLeft />
                        </button>
                    )}
                    <h1 className="text-2xl font-heading font-bold text-white italic tracking-wider">
                ...
                {view === 'feed' && (
                    <button
                        ...
                        className="bg-f1-red hover:bg-red-600 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2"
                    >
            ...
            {showCreate && (
                <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
                    <div className="bg-[#1A1A20] p-6 rounded-2xl w-full max-w-lg border border-[#333]">
                        <h2 className="text-xl font-bold text-white mb-4">Start a Discussion</h2>
                        <input
                            className="w-full bg-[#0B0B0F] border border-[#333] p-3 rounded-lg text-white mb-3 font-bold"
                            placeholder="Title"
                            value={newTitle}
                            onChange={e => setNewTitle(e.target.value)}
                        />
                        <textarea
                            className="w-full bg-[#0B0B0F] border border-[#333] p-3 rounded-lg text-white mb-4 h-32"
                        ...
                        <div className="flex justify-end gap-3">
                            ...
                            <button onClick={handleCreateThread} className="bg-f1-red text-white px-6 py-2 rounded-lg font-bold">Post</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                {view === 'feed' ? (
                    <div className="space-y-3">
                        {threads.map(t => (
                            <div
                                ...
                                className="bg-[#1A1A20] hover:bg-[#222] border border-[#2A2A30] hover:border-f1-red/30 p-4 rounded-xl cursor-pointer transition-all group flex gap-4"
                            >
                                {/* Vote Column */}
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex items-center gap-3 text-xs text-gray-500">
                                        ...
                                        <span>•</span>
                                        <span className="bg-[#111] px-2 py-0.5 rounded text-gray-400">{t.category}</span>
                                        <span>•</span>
                    ...
                    activeThread && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            {/* Original Post */}
                            <div className="bg-[#1A1A20] border border-[#2A2A30] p-6 rounded-2xl relative overflow-hidden">
                            ...
                            <div>
                                ...
                                <div className="space-y-4 mt-6">
                                    ...
                                    {activeThread.comments.map(c => (
                                        <div key={c.id} className="bg-[#111] p-4 rounded-xl border border-[#222]">
...
function CommentComposer({ onSubmit }) {
    ...
    return (
        <div className="bg-[#1A1A20] p-4 rounded-xl border border-[#2A2A30] flex gap-3">
            <textarea
                className="flex-1 bg-transparent text-white resize-none focus:outline-none h-10 min-h-[40px] pt-2"

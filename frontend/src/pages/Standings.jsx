                ...
                <div className="flex items-end justify-between border-b-2 border-black pb-2 shrink-0 gap-8">
                    ...
                    <div className="flex flex-col gap-1 pb-1">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
                            <h1 className="text-2xl md:text-4xl font-heading text-black uppercase leading-none">Season Standings</h1>
                            <select
                                value={year}
                                onChange={(e) => setYear(Number(e.target.value))}
                                className="bg-white border-2 border-black text-black font-body text-base md:text-lg px-2 py-1 md:py-0.5 outline-none shadow-hard-sm cursor-pointer"
                            >
                                {availableYears.map(y => <option key={y} value={y}>{y}</option>)}
                            </select>
                        </div>
                        <div className="flex items-center gap-2 md:gap-3 w-full sm:w-auto">
                            <div className="h-3 w-32 md:w-48 border-2 border-black bg-white p-0.5">
                                <div className="h-full bg-f1-red" style={{ width: `${(completedRounds / totalRounds) * 100}%` }} />
                            </div>
                            <span className="text-xs md:text-sm font-bold opacity-60">{completedRounds}/{totalRounds} ROUNDS</span>
                        </div>
                    </div>
                </div>
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
                                <table className="w-full text-left border-collapse">
                                    <thead className="sticky top-0 bg-f1-light border-b-4 border-black z-10">
                                        <tr className="text-sm md:text-lg font-heading text-black uppercase">
                                    ...
                                    <tbody className="divide-y-2 divide-gray-200">
                                        {data.drivers.map((d, i) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <td className="p-2 md:p-3 text-base md:text-xl font-heading">
                                                        <span className={cn(
                                                    ...
                                                    <td className="p-2 md:p-3 text-right text-sm md:text-lg font-mono">
                                                        {i > 0 ? `-${gap}` : '—'}
                                                    </td>
                                                </tr>
                                            );
                                        ...
                                        {data.constructors?.map((c, i) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="p-2 md:p-3 text-right text-base md:text-xl font-heading">
                                                        <span className={cn(i === 0 && "text-xl md:text-2xl font-black")}>
                                                            {c.points}
                                                        </span>
                                                    </td>
                                                    <td className="p-2 md:p-3 text-right text-sm md:text-lg font-mono">{c.wins}</td>
                                                </tr>
                                            );
                                        })}
                                    </tbody>

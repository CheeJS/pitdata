...
def fetch_and_store_results(year, race, session_type, session_db):
    type_map = {'R': 'Race', 'S': 'Sprint', 'Q': 'Qualifying'}
    label = type_map.get(session_type, session_type)
    
    # Check bounds
    existing = session_db.query(Result).filter_by(race_id=race.id, session_type=session_type).count()
    if existing > 0:
        return f"Skipped (Exists: {existing})"

    try:
        # For Sprints, FastF1 uses 'Sprint', for Qualy 'Q', Race 'R'
        ff1_identifier = 'Sprint' if session_type == 'S' else session_type
        
        # Verify Session Exists in Schedule (avoid errors for races without sprints)
        if session_type == 'S' and not race.sprint_date:
             return "Skipped (No Sprint)"
...
def seed_results(year, session_db):
    ...
    print(f"{'Round':<5} | {'Race':<25} | {'Race':<15} | {'Sprint':<15} | {'Qualifying':<15}")
    print("-" * 85)
    
    for race in races:
        # Race
        r_res = fetch_and_store_results(year, race, 'R', session_db)
        # Sprint
        s_res = fetch_and_store_results(year, race, 'S', session_db)
        # Qualifying
        q_res = fetch_and_store_results(year, race, 'Q', session_db)
        
        print(f"{race.round:<5} | {race.race_name:<25} | {r_res:<15} | {s_res:<15} | {q_res:<15}")
        session_db.commit() # Commit per race
